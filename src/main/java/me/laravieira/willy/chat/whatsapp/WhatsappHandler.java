...
public class WhatsappHandler {
    public static ErrorHandler.Result onError(it.auties.whatsapp.api.Whatsapp ignore, ErrorHandler.Location location, Throwable throwable) {
        ...
        if(location.name().equals("LOGGED_OUT")) {
            ...
            Willy.getLogger().severe("WA is disconnected: "+location.name());
        ...
        Willy.getLogger().warning(location.name()+": "+throwable.getMessage());
        return ErrorHandler.Result.DISCARD;
    }

    public static void onQRCode(Path path) {
        try {
            Willy.getLogger().fine("Sending WhatsApp QR-Code to Discord...");
