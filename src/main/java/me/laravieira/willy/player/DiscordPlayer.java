...
public class DiscordPlayer {
	...
	public static DiscordPlayer createDiscordPlayer(VoiceChannel channel) {
		...
		if(!player.getChannel().isMemberConnected(Snowflake.of(Config.getDiscordID())).block()) {
			player.getChannel().join(spec -> spec.setProvider(player.getProvider())).block();
			MyLogger.getConsoleLogger().info("Joined on "+player.getChannel().getName()+" ("+player.getChannel().getId().asString()+").");
		}return player;
	}
	...
	public void destroy() {
		...
		if(channel.isMemberConnected(Snowflake.of(Config.getDiscordID())).block())
	...
	public static void onVoiceChannelUpdate(VoiceStateUpdateEvent event) {
		...
		if(players.containsKey(guild)) {
			...
			if(voiceStates.count().block() == 1) {
				if(voiceStates.blockFirst().getUserId().equals(Snowflake.of(Config.getDiscordID()))) {
					players.get(guild).destroy();
					MyLogger.getConsoleLogger().info("Willy was alone on a voice channel, player instance destroed.");
				}
			}
		}
	}
